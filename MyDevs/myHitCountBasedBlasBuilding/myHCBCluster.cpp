...
*
* Summary:
* Hit Count Basaed BLAS Building, idea from GDC2025 - RTX Mega Geometry(https://youtu.be/KblmxDkaUfc?t=2807)
* * - clas template (x) , implicit build(o)
* This work continues from the "MyHCBCluster" implementation.
*
...
void MyHCBCluster::createAccelerationStructureBuffer(AccelerationStructure& accelerationStructure,
...
void MyHCBCluster::initCLASes()
{
	...
	//clasInputInfo.flags = VK_BUILD_ACCELERATION_STRUCTURE_PREFER_FAST_BUILD_BIT_KHR; // TODO TEPM
...
void MyHCBCluster::initClusteredBLASes()
{
	...
	//clusteredBlasInputInfo.flags = VK_BUILD_ACCELERATION_STRUCTURE_PREFER_FAST_BUILD_BIT_KHR; // TODO TEMP
...
void MyHCBCluster::buildCLASes(VkCommandBuffer cmdBuffer)
{
	...
	//inputInfo.flags = VK_BUILD_ACCELERATION_STRUCTURE_PREFER_FAST_BUILD_BIT_KHR; // TODO TEMP
...
void MyHCBCluster::buildClusteredBLASes(VkCommandBuffer cmdBuffer)
{
...
void MyHCBCluster::buildTLAS(VkCommandBuffer cmdBuffer)
{
...
void MyHCBCluster::createShaderBindingTables()
{
...
void MyHCBCluster::createRayTracingPipeline()
{
	...
	*/
	...
	{
		shaderStages.push_back(loadShader(getShadersPath() + "MyHCBCluster/raygen.rgen.spv", VK_SHADER_STAGE_RAYGEN_BIT_KHR));
	...
	// Miss group
	{
		shaderStages.push_back(loadShader(getShadersPath() + "MyHCBCluster/miss.rmiss.spv", VK_SHADER_STAGE_MISS_BIT_KHR));
		VkRayTracingShaderGroupCreateInfoKHR shaderGroup{};
		shaderGroup.sType = VK_STRUCTURE_TYPE_RAY_TRACING_SHADER_GROUP_CREATE_INFO_KHR;
		shaderGroup.type = VK_RAY_TRACING_SHADER_GROUP_TYPE_GENERAL_KHR;
		shaderGroup.generalShader = static_cast<uint32_t>(shaderStages.size()) - 1;
		...
		shaderStages.push_back(loadShader(getShadersPath() + "MyHCBCluster/shadow.rmiss.spv", VK_SHADER_STAGE_MISS_BIT_KHR));
		shaderGroup.generalShader = static_cast<uint32_t>(shaderStages.size()) - 1;
		shaderGroups.push_back(shaderGroup);
	}
	...
	{
		shaderStages.push_back(loadShader(getShadersPath() + "MyHCBCluster/closesthit.rchit.spv", VK_SHADER_STAGE_CLOSEST_HIT_BIT_KHR));
		VkRayTracingShaderGroupCreateInfoKHR shaderGroup{};
		shaderGroup.sType = VK_STRUCTURE_TYPE_RAY_TRACING_SHADER_GROUP_CREATE_INFO_KHR;
		...
		shaderStages.push_back(loadShader(getShadersPath() + "MyHCBCluster/anyhit.rahit.spv", VK_SHADER_STAGE_ANY_HIT_BIT_KHR));
...
void MyHCBCluster::createDescriptorSets()
{
...
void MyHCBCluster::createUniformBuffer()
{
...
void MyHCBCluster::handleResize()
{
	// Recreate image
...
void MyHCBCluster::buildCommandBuffers()
{
	if (resized)
	{
		handleResize();
...
void MyHCBCluster::updateUniformBuffers()
{
...
void MyHCBCluster::getEnabledFeatures()
{
...
void MyHCBCluster::enableExtensions()
{
...
void MyHCBCluster::prepare()
{
	...
	blasUpdateComputePass->createPipeline(getShadersPath() + "MyHCBCluster/clusteredBlasUpdate.comp.spv");
	animComputePass = std::make_unique<MyAnimComputePass>(device);
	animComputePass->createDescriptorSets(model);
	animComputePass->createPipeline(getShadersPath() + "MyHCBCluster/anim.comp.spv");
...
void MyHCBCluster::OnUpdateUIOverlay(vks::UIOverlay* overlay)
{
