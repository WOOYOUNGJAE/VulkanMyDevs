...
std::vector<const char*> MyVulkanBase::args;

VkResult MyVulkanBase::createInstance()
{
...
std::string MyVulkanBase::getWindowTitle() const
{
...
void MyVulkanBase::createCommandBuffers()
{
...
void MyVulkanBase::destroyCommandBuffers()
{
	vkFreeCommandBuffers(device, cmdPool, static_cast<uint32_t>(drawCmdBuffers.size()), drawCmdBuffers.data());
}

std::string MyVulkanBase::getShadersPath() const
{
	return getShaderBasePath() + shaderDir + "/";
}

void MyVulkanBase::createPipelineCache()
{
...
VkPipelineShaderStageCreateInfo MyVulkanBase::loadShader(std::string fileName, VkShaderStageFlagBits stage)
{
...
void MyVulkanBase::renderLoop()
{
	// SRS - for non-apple plaforms, handle benchmarking here within MyVulkanBase::renderLoop()
...
void MyVulkanBase::updateOverlay()
{
	if (!settings.overlay)
		return;
...
void MyVulkanBase::drawUI(const VkCommandBuffer commandBuffer)
{
...
void MyVulkanBase::prepareFrame()
{
...
void MyVulkanBase::setupConsole(std::string title)
{
	AllocConsole();
...
void MyVulkanBase::setupDPIAwareness()
{
...
HWND MyVulkanBase::setupWindow(HINSTANCE hinstance, WNDPROC wndproc)
{
...
void MyVulkanBase::handleMessages(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	switch (uMsg)
	{
	case WM_CLOSE:
...
#elif defined(VK_USE_PLATFORM_ANDROID_KHR)
int32_t MyVulkanBase::handleAppInput(struct android_app* app, AInputEvent* event)
{
	MyVulkanBase* vulkanExample = reinterpret_cast<MyVulkanBase*>(app->userData);
...
void MyVulkanBase::handleAppCommand(android_app* app, int32_t cmd)
{
	...
	void* displayLinkContext)
{
	@autoreleasepool
	{
		auto vulkanExample = static_cast<MyVulkanBase*>(displayLinkContext);
			vulkanExample->displayLinkOutputCb();
	}
...
// SRS - Window resizing already handled by windowResize() in MyVulkanBase::submitFrame()
...
void* MyVulkanBase::setupWindow(void* view)
{
#if defined(VK_EXAMPLE_XCODE_GENERATED)
...
void MyVulkanBase::displayLinkOutputCb()
{
#if defined(VK_EXAMPLE_XCODE_GENERATED)
	if (benchmark.active) {
...
void MyVulkanBase::mouseDragged(float x, float y)
{
	handleMouseMove(static_cast<uint32_t>(x), static_cast<uint32_t>(y));
}

void MyVulkanBase::windowWillResize(float x, float y)
{
	resizing = true;
	if (prepared)
	{
...
void MyVulkanBase::windowDidResize()
{
	resizing = false;
}
#elif defined(_DIRECT2DISPLAY)
#elif defined(VK_USE_PLATFORM_DIRECTFB_EXT)
IDirectFBSurface* MyVulkanBase::setupWindow()
{
	DFBResult ret;
	int posx = 0, posy = 0;
...
void MyVulkanBase::handleEvent(const DFBWindowEvent* event)
{
	switch (event->type)
	{
	case DWET_CLOSE:
...
#elif defined(VK_USE_PLATFORM_WAYLAND_KHR)
/*static*/void MyVulkanBase::registryGlobalCb(void* data,
	wl_registry* registry, uint32_t name, const char* interface,
	uint32_t version)
{
	MyVulkanBase* self = reinterpret_cast<MyVulkanBase*>(data);
	self->registryGlobal(registry, name, interface, version);
}

/*static*/void MyVulkanBase::seatCapabilitiesCb(void* data, wl_seat* seat,
	uint32_t caps)
{
	MyVulkanBase* self = reinterpret_cast<MyVulkanBase*>(data);
...
/*static*/void MyVulkanBase::pointerEnterCb(void* data,
	wl_pointer* pointer, uint32_t serial, wl_surface* surface,
	wl_fixed_t sx, wl_fixed_t sy)
{
}

/*static*/void MyVulkanBase::pointerLeaveCb(void* data,
	wl_pointer* pointer, uint32_t serial, wl_surface* surface)
{
}

/*static*/void MyVulkanBase::pointerMotionCb(void* data,
	wl_pointer* pointer, uint32_t time, wl_fixed_t sx, wl_fixed_t sy)
{
	MyVulkanBase* self = reinterpret_cast<MyVulkanBase*>(data);
	self->pointerMotion(pointer, time, sx, sy);
}
void MyVulkanBase::pointerMotion(wl_pointer* pointer, uint32_t time, wl_fixed_t sx, wl_fixed_t sy)
{
	handleMouseMove(wl_fixed_to_int(sx), wl_fixed_to_int(sy));
}

/*static*/void MyVulkanBase::pointerButtonCb(void* data,
...
void MyVulkanBase::pointerButton(struct wl_pointer* pointer,
...
/*static*/void MyVulkanBase::pointerAxisCb(void* data,
...
void MyVulkanBase::pointerAxis(wl_pointer* pointer, uint32_t time,
...
/*static*/void MyVulkanBase::keyboardKeymapCb(void* data,
	struct wl_keyboard* keyboard, uint32_t format, int fd, uint32_t size)
{
}

/*static*/void MyVulkanBase::keyboardEnterCb(void* data,
	struct wl_keyboard* keyboard, uint32_t serial,
	struct wl_surface* surface, struct wl_array* keys)
{
}

/*static*/void MyVulkanBase::keyboardLeaveCb(void* data,
	struct wl_keyboard* keyboard, uint32_t serial,
	struct wl_surface* surface)
{
}

/*static*/void MyVulkanBase::keyboardKeyCb(void* data,
...
void MyVulkanBase::keyboardKey(struct wl_keyboard* keyboard,
...
/*static*/void MyVulkanBase::keyboardModifiersCb(void* data,
	struct wl_keyboard* keyboard, uint32_t serial, uint32_t mods_depressed,
	uint32_t mods_latched, uint32_t mods_locked, uint32_t group)
{
}

void MyVulkanBase::seatCapabilities(wl_seat* seat, uint32_t caps)
{
...
void MyVulkanBase::registryGlobal(wl_registry* registry, uint32_t name,
...
/*static*/void MyVulkanBase::registryGlobalRemoveCb(void* data,
	struct wl_registry* registry, uint32_t name)
{
}

void MyVulkanBase::initWaylandConnection()
{
...
void MyVulkanBase::setSize(int width, int height)
{
	if (width <= 0 || height <= 0)
		return;
...
xdg_surface_handle_configure(void* data, struct xdg_surface* surface,
	uint32_t serial)
{
	MyVulkanBase* base = (MyVulkanBase*)data;
...
struct xdg_surface* MyVulkanBase::setupWindow()
{
...
xcb_window_t MyVulkanBase::setupWindow()
{
...
void MyVulkanBase::initxcbConnection()
{
...
void MyVulkanBase::handleEvent(const xcb_generic_event_t* event)
{
...
void MyVulkanBase::keyPressed(uint32_t) {}

void MyVulkanBase::mouseMoved(double x, double y, bool& handled) {}

void MyVulkanBase::buildCommandBuffers() {}

void MyVulkanBase::createSynchronizationPrimitives()
{
...
void MyVulkanBase::createCommandPool()
{
...
void MyVulkanBase::setupDepthStencil()
{
...
void MyVulkanBase::setupFrameBuffer()
{
...
void MyVulkanBase::setupRenderPass()
{
...
void MyVulkanBase::getEnabledFeatures() {}

void MyVulkanBase::getEnabledExtensions() {}

void MyVulkanBase::windowResize()
{
	if (!prepared)
	{
		return;
...
void MyVulkanBase::handleMouseMove(int32_t x, int32_t y)
{
...
void MyVulkanBase::windowResized() {}

void MyVulkanBase::createSurface()
{
#if defined(_WIN32)
...
void MyVulkanBase::createSwapChain()
{
	swapChain.create(width, height, settings.vsync, settings.fullscreen);
}

void MyVulkanBase::OnUpdateUIOverlay(vks::UIOverlay* overlay) {}

#if defined(_WIN32)
void MyVulkanBase::OnHandleMessage(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {};
#endif
