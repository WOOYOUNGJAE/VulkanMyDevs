...
void MyDynamicAccelerationStructure::createAccelerationStructureBuffer(AccelerationStructure& accelerationStructure,
...
void MyDynamicAccelerationStructure::initBLASes()
{
	...
	for (auto node : model.linearNodes)
	{
		if (node->mesh)
		{
			...
			ASBuildInfo& refBlasBuildInfo = asBuildInfos.back(); // avoid dangling pointer due to moving array;
...
void MyDynamicAccelerationStructure::buildBLASes()
{
	...
	for (uint32_t blasIdx = 0; blasIdx < numBlases; ++blasIdx)
	{
		...
		ASBuildInfo& refBuildInfo = asBuildInfos[blasIdx];
		if (isFirstBuild)
		{
...
void MyDynamicAccelerationStructure::buildTLAS()
{
...
void MyDynamicAccelerationStructure::createRayTracingPipeline()
{
	...
	*/
	...
	{
		shaderStages.push_back(loadShader(getShadersPath() + "myDynamicAccelerationStructure/raygen.rgen.spv", VK_SHADER_STAGE_RAYGEN_BIT_KHR));
	...
	// Miss group
	{
		shaderStages.push_back(loadShader(getShadersPath() + "myDynamicAccelerationStructure/miss.rmiss.spv", VK_SHADER_STAGE_MISS_BIT_KHR));
		VkRayTracingShaderGroupCreateInfoKHR shaderGroup{};
		shaderGroup.sType = VK_STRUCTURE_TYPE_RAY_TRACING_SHADER_GROUP_CREATE_INFO_KHR;
		shaderGroup.type = VK_RAY_TRACING_SHADER_GROUP_TYPE_GENERAL_KHR;
		shaderGroup.generalShader = static_cast<uint32_t>(shaderStages.size()) - 1;
		...
		shaderStages.push_back(loadShader(getShadersPath() + "myDynamicAccelerationStructure/shadow.rmiss.spv", VK_SHADER_STAGE_MISS_BIT_KHR));
		shaderGroup.generalShader = static_cast<uint32_t>(shaderStages.size()) - 1;
		shaderGroups.push_back(shaderGroup);
	}
	...
	{
		shaderStages.push_back(loadShader(getShadersPath() + "myDynamicAccelerationStructure/closesthit.rchit.spv", VK_SHADER_STAGE_CLOSEST_HIT_BIT_KHR));
		VkRayTracingShaderGroupCreateInfoKHR shaderGroup{};
		shaderGroup.sType = VK_STRUCTURE_TYPE_RAY_TRACING_SHADER_GROUP_CREATE_INFO_KHR;
		...
		shaderStages.push_back(loadShader(getShadersPath() + "myDynamicAccelerationStructure/anyhit.rahit.spv", VK_SHADER_STAGE_ANY_HIT_BIT_KHR));
...
void MyDynamicAccelerationStructure::createDescriptorSets()
{
	...
	std::vector<VkWriteDescriptorSet> writeDescriptorSets = {
		...
		// Binding 4: Geometry node information SSBO
		vks::initializers::writeDescriptorSet(descriptorSet, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 4, &model.geometryNodes.descriptor),
		// Binding 5 : All Mesh Primitives
		vks::initializers::writeDescriptorSet(descriptorSet, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 5, &model.primitives.descriptor)
	};
...
void MyDynamicAccelerationStructure::createUniformBuffer()
{
	VK_CHECK_RESULT(vulkanDevice->createBuffer(
...
void MyDynamicAccelerationStructure::handleResize()
{
	// Recreate image
...
void MyDynamicAccelerationStructure::buildCommandBuffers()
{
	if (resized)
	{
		handleResize();
...
void MyDynamicAccelerationStructure::updateUniformBuffers()
{
...
void MyDynamicAccelerationStructure::getEnabledFeatures()
{
...
void MyDynamicAccelerationStructure::draw()
{
...
void MyDynamicAccelerationStructure::render()
{
	if (!prepared)
		return;
	updateUniformBuffers();
...
MyDynamicAccelerationStructure* myClusterAccelerationStructureNV;
...
#endif
	for (int32_t i = 0; i < __argc; i++) { MyDynamicAccelerationStructure::args.push_back(__argv[i]); };
	myClusterAccelerationStructureNV = new MyDynamicAccelerationStructure();
