...
void MyClusteredSkeletalMesh::createAccelerationStructureBuffer(AccelerationStructure& accelerationStructure,
...
void MyClusteredSkeletalMesh::initCLASes()
{
...
void MyClusteredSkeletalMesh::initClusteredBLASes()
{
...
void MyClusteredSkeletalMesh::initTLAS()
{
...
void MyClusteredSkeletalMesh::createShaderBindingTables()
{
...
void MyClusteredSkeletalMesh::createRayTracingPipeline()
{
	...
	*/
	...
	{
		shaderStages.push_back(loadShader(getShadersPath() + "MyClusteredSkeletalMesh/raygen.rgen.spv", VK_SHADER_STAGE_RAYGEN_BIT_KHR));
	...
	// Miss group
	{
		shaderStages.push_back(loadShader(getShadersPath() + "MyClusteredSkeletalMesh/miss.rmiss.spv", VK_SHADER_STAGE_MISS_BIT_KHR));
		VkRayTracingShaderGroupCreateInfoKHR shaderGroup{};
		shaderGroup.sType = VK_STRUCTURE_TYPE_RAY_TRACING_SHADER_GROUP_CREATE_INFO_KHR;
		shaderGroup.type = VK_RAY_TRACING_SHADER_GROUP_TYPE_GENERAL_KHR;
		shaderGroup.generalShader = static_cast<uint32_t>(shaderStages.size()) - 1;
		...
		shaderStages.push_back(loadShader(getShadersPath() + "MyClusteredSkeletalMesh/shadow.rmiss.spv", VK_SHADER_STAGE_MISS_BIT_KHR));
		shaderGroup.generalShader = static_cast<uint32_t>(shaderStages.size()) - 1;
		shaderGroups.push_back(shaderGroup);
	}
	...
	{
		shaderStages.push_back(loadShader(getShadersPath() + "MyClusteredSkeletalMesh/closesthit.rchit.spv", VK_SHADER_STAGE_CLOSEST_HIT_BIT_KHR));
		VkRayTracingShaderGroupCreateInfoKHR shaderGroup{};
		shaderGroup.sType = VK_STRUCTURE_TYPE_RAY_TRACING_SHADER_GROUP_CREATE_INFO_KHR;
		...
		shaderStages.push_back(loadShader(getShadersPath() + "MyClusteredSkeletalMesh/anyhit.rahit.spv", VK_SHADER_STAGE_ANY_HIT_BIT_KHR));
...
void MyClusteredSkeletalMesh::createDescriptorSets()
{
...
void MyClusteredSkeletalMesh::createUniformBuffer()
{
...
void MyClusteredSkeletalMesh::handleResize()
{
	// Recreate image
...
void MyClusteredSkeletalMesh::buildCommandBuffers()
{
	if (resized)
	{
		handleResize();
...
void MyClusteredSkeletalMesh::updateUniformBuffers()
{
...
void MyClusteredSkeletalMesh::getEnabledFeatures()
{
...
void MyClusteredSkeletalMesh::prepare()
{
	...
	animComputePass->createPipeline(getShadersPath() + "MyClusteredSkeletalMesh/anim.comp.spv");
...
void MyClusteredSkeletalMesh::draw()
{
...
void MyClusteredSkeletalMesh::render()
{
	if (!prepared)
		return;
...
#endif
	for (int32_t i = 0; i < __argc; i++) { MyClusteredSkeletalMesh::args.push_back(__argv[i]); };
	myClusterAccelerationStructureNV = new MyClusteredSkeletalMesh();
